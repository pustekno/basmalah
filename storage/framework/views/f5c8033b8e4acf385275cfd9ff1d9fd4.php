            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
                ...
                <div class="px-6 py-5 border-b border-gray-100 dark:border-slate-700">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><?php echo e($user->name); ?></h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400"><?php echo e($user->email); ?></p>
                        </div>
                    </div>
                </div>

                <form method="POST" action="<?php echo e(route('admin.users.update', $user)); ?>" class="p-6" x-data="{ 
                    selectedRoles: <?php echo e(json_encode($user->getRoleNames()->toArray())); ?>,
                    selectedMasjid: <?php echo json_encode($user->masjid_id ? $masjids->find($user->masjid_id)?->name : null, 15, 512) ?>,
                    selectedMasjidId: <?php echo json_encode($user->masjid_id ? (int)$user->masjid_id : null, 15, 512) ?>,
                    openMasjid: false,
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="mb-6" x-show="needsMasjid()" x-transition>
                        ...
                        <!-- Custom Dropdown -->
                        <div class="relative">
                            <input type="hidden" name="masjid_id" id="masjid_id" value="<?php echo e(old('masjid_id', $user->masjid_id)); ?>">
                            ...
                            >
                                <div class="overflow-y-auto" style="max-height: 240px; scrollbar-width: thin; -webkit-overflow-scrolling: touch;">
                                    ...
                                    <?php $__currentLoopData = $masjids; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $masjid): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <button 
                                            type="button"
                                            onclick="document.getElementById('masjid_id').value = <?php echo e($masjid->id); ?>; document.querySelector('[x-data]').__x.$data.selectedMasjid = '<?php echo e(addslashes($masjid->name)); ?>'; document.querySelector('[x-data]').__x.$data.selectedMasjidId = <?php echo e($masjid->id); ?>; document.querySelector('[x-data]').__x.$data.openMasjid = false"
...
<?php endif; ?>
<?php /**PATH D:\laragon\www\project-basmalah\basmallah\resources\views/admin/users/edit.blade.php ENDPATH**/ ?>