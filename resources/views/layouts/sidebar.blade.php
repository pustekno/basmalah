...
<aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 transform transition-transform duration-300 ease-in-out" 
    ...
    <div class="flex flex-col h-full">
        ...
        <div class="bg-blue-600 dark:bg-blue-700 px-4 py-3">
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                    ...
                    <div class="text-white">
                        ...
                        <div class="text-sm font-bold">{{ session('impersonating_user_name') }}</div>
                    </div>
                </div>
                <form method="POST" action="{{ route('admin.users.back-to-admin') }}">
                    @csrf
            ...
            @if(session('impersonating_roles'))
            <div class="flex flex-wrap gap-1">
                @foreach(session('impersonating_roles') as $role)
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-1">
            <!-- Dashboard -->
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-3 rounded-xl transition-all {{ request()->routeIs('dashboard') ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/50' }}" title="{{ __('navigation.dashboard') }}">
            ...
            @can('view categories')
            <!-- Categories -->
            <a href="{{ route('categories.index') }}" class="flex items-center gap-3 px-3 py-3 rounded-xl transition-all {{ request()->routeIs('categories.*') ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/50' }}" title="{{ __('navigation.categories') }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @can('view budgets')
            <!-- Budgets -->
            <a href="{{ route('budgets.index') }}" class="flex items-center gap-3 px-3 py-3 rounded-xl transition-all {{ request()->routeIs('budgets.*') ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/50' }}" title="{{ __('navigation.budgets') }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-all">
                    <div class="w-10 h-10 bg-yellow-600 rounded-full flex items-center justify-center text-white font-semibold">
                        {{ substr(session('is_impersonating') ? session('impersonating_user_name') : Auth::user()->name, 0, 1) }}
                    </div>
                    <div class="flex-1 text-left">
                        <div class="text-sm font-semibold text-gray-900 dark:text-white">{{ session('is_impersonating') ? session('impersonating_user_name') : Auth::user()->name }}</div>
