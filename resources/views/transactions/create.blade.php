    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                <form action="{{ route('transactions.store') }}" method="POST" enctype="multipart/form-data" x-data="transactionForm()">
                    @csrf
                                ...
                                <div class="relative">
                                    ...
                                    <select name="account_id" id="account_id" required
                                        ...
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}" {{ old('account_id', $selectedAccount) == $account->id ? 'selected' : '' }}>
                                                {{ $account->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-amber-800/10 rounded-2xl p-5 border border-amber-200 dark:border-amber-800/30">
                            <label class="flex items-start gap-4 cursor-pointer">
                                <div class="relative flex items-center">
                                    <input type="checkbox" name="upcoming_flag" value="1" {{ old('upcoming_flag') ? 'checked' : '' }}
                                        class="sr-only peer">
